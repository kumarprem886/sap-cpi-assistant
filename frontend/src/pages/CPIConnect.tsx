...
export default function CPIConnect() {
  ...
  const importOne = async (idx: number, pkg: string, fileObj?: File) => {
    ...
    try {
      ...
      const r = await cpiAPI.importZip(file, pkg)
          ...
          {activeTab === 'import' && (
            <div className="space-y-5">
              <div>
                ...
                {pkgLoading ? <div className="flex items-center gap-2 text-gray-400 text-sm py-2"><Loader2 size={14} className="animate-spin" /> Loading…</div>
                  : packages.length === 0 ? <p className="text-sm text-yellow-300">No packages found — visit the Packages tab first.</p>
                  : (
                    <select className="select-field w-full max-w-md" value={importPkg} onChange={e => setImportPkg(e.target.value)}>
              ...
              <div>
                <label className="block text-sm font-medium text-gray-300 mb-1.5">iFlow ZIP files</label>
                <div onClick={() => importZipRef.current?.click()} onDragOver={e => e.preventDefault()}
                  ...
                  <p className="text-gray-500 text-xs mt-1">Metadata is read from each ZIP automatically</p>
                </div>
